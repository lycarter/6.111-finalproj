...
%also navigate quickly through the file using ctrl+arrow_keys
...
[picture] = imread('labkit_image.bmp');
...
[picture color_table] = imread('labkit_image.bmp');
...
%open a file
output_name = 'labkit_image_color_table_red.coe';
...
%open a file
output_name = 'labkit_image_color_table_green.coe';
file = fopen(output_name,'w');
...
%open a file
output_name = 'labkit_image_color_table_blue.coe';
file = fopen(output_name,'w');
...
tiger = regen_picture(:,:);             %grabs the red part of the colortable
disp(tiger);
%scaled_data = tiger*255;              %scales the floats back to 0-255
%rounded_data = round(scaled_data);  %rounds them down
data = dec2bin(tiger,8);     %convert the binary data to 8 bit binary #s

%open a file
output_name = 'labkit_image.coe';
file = fopen(output_name,'w');
